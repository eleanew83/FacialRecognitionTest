...
DESTINATION_DIR = os.path.join(os.path.dirname(YOLO_BASE), "yolo_detection")
TRAIN_RATIO = 0.8
VAL_RATIO = 0.2
...
def create_dataset_structure():
    ...
    for split in ['train', 'val']:
        for folder in ['images', 'labels']:
            path = os.path.join(DESTINATION_DIR, folder, split)
            os.makedirs(path, exist_ok=True)
    
    # Create a dataset.yaml file for YOLO
    yaml_content = f"""
